...
public class SampleIT {
    ...
    public static Archive<?> getEarArchive() {
        ...
        EnterpriseArchive ear = ShrinkWrap.create(EnterpriseArchive.class, "maven-kurs-ear.ear");

        // Current directory is the root of the "maven-kurs-web" project. Go up one level
        // and enter the "maven-kurs-ejb" project.
        // The ejb jar is found in the "target" directory:
        File f = new File("../maven-kurs-ejb/target/maven-kurs-ejb.jar");
        JavaArchive ejbJar = ShrinkWrap.create(ZipImporter.class, "maven-kurs-ejb.jar").importFrom(f).as(JavaArchive.class);
        ear.addAsModule(ejbJar);

        // Now grab the web archive:
        f = new File("./target/maven-kurs-web.war");
        if (f.exists() == false) {
            throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
        }
        WebArchive war = ShrinkWrap.create(ZipImporter.class, "maven-kurs-web.war").importFrom(f).as(WebArchive.class);
        ear.addAsModule(war);
